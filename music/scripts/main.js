...
Events.on(ClientLoadEvent, () => {
	...
	var play = (music) => {
		...
		nowplaying.play();
		Vars.ui.hudfrag.showToast("Now playing \n\"" + music.parent().name() + ":" + music.name() + "\"");
		return true;
	}
	var stop = () => {
	...
	var playRandom = (musicarray) => {
		...
		if (musicarray[0] != null) play(musicarray[Math.floor(Math.random()*musicarray.length)]);
	}
